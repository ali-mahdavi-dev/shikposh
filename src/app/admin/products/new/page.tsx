...
import { createProduct as createProductAction } from '../_actions/product.actions';
...
export default function NewProductPage() {
  ...
  const handleSubmit = async (values: CreateProductRequest) => {
    try {
      ...
      // Option 1: Use server action (recommended)
      const result = await createProductAction(validationResult.data);
      if (result.success) {
        message.success('محصول با موفقیت ایجاد شد');
        router.push('/admin/products');
      } else {
        message.error(result.error || 'خطا در ایجاد محصول');
      }

      // Option 2: Use mutation hook (alternative)
      // await createProductMutation.mutateAsync(validationResult.data);
    } catch (error) {
      // Use enterprise error handling
      const appError = handleError(error);
      message.error(appError.message);
      console.error('Product creation error:', appError.toJSON());
    }
  }; {
    try {
