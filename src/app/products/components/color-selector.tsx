...
const ColorSelector: React.FC<ColorSelectorProps> = ({ colors, selectedColor, onColorChange }) => {
  return (
    <div className="space-y-3">
      <Text strong className="text-gray-800">رنگ: <span className="font-normal text-gray-600">{colors[selectedColor].name}</span></Text>
      <div className="flex flex-wrap gap-3">
        {Object.entries(colors).map(([colorKey, color]: [string, ProductColor]) => (
          <Tooltip 
            key={colorKey} 
            title={
          ...
          >
            <motion.div
              className={`w-10 h-10 rounded-full border-2 cursor-pointer flex items-center justify-center transition-all duration-200\n                ${colorKey === selectedColor ? 'border-pink-500 shadow-md' : 'border-gray-300 hover:border-pink-300'}\n                ${color.stock === 0 ? 'opacity-50 cursor-not-allowed' : ''}\n              `}
              style={{ backgroundColor: colorKey === 'red' ? '#ef4444' : colorKey === 'blue' ? '#3b82f6' : colorKey === 'black' ? '#1f2937' : '#d1d5db' }}
