...
export const api = {
  ...
  getFeaturedProducts: () => fetch(`${API_BASE_URL}/products?isFeatured=true`).then(res => res.json()),
  ...
  // Reviews
  getReviews: (productId: string) => fetch(`${API_BASE_URL}/reviews?productId=${productId}`).then(res => res.json()),
  createReview: (review: { productId: string; user: string; rating: number; comment: string; date: string }) =>
